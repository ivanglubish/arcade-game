...
Enemy.prototype.update = function (dt) {
  ...
  if (player.x < this.x + 60 &&
    ...
    player.x = 200;
    player.y = 390;
    player.sprite = player.boy; // change player back to char-boy
  }
};
...
Player.prototype.update = function (keyPress) {
  ...
  // if player reach a woter on third block move player back to position and change to cat-girl
  if (this.y < 0 && this.x == 200) {  // don't cross canvas up
    //this.player = 100;
    this.y = 390;
    this.sprite = this.cat;
  ...
  if (this.y < 0 && this.x == 300) {  // don't cross canvas up
    //this.player = 300;
    this.y = 390;
    this.sprite = this.horn;
    alert('you are a horn girl');
  }

  // if player reach woter on second block move player back to a new position and change to pink
  if (this.y < 0 && this.x == 100) {  // don't cross canvas up
    //this.player = 100;
    this.y = 390;
    this.sprite = this.pink;
  ...
  if (this.y < 0 && this.x == 400) {  // don't cross canvas up
    //this.player = 400;
    this.y = 390;
    this.sprite = this.boy;
    alert('you are a boy');
  }

  // If player reach woter on first block move player back to a new position and change to princess
  if (this.y < 0 && this.x == 0) {  // don't cross canvas up
    //this.player = 0;
    this.y = 390;
    this.sprite = this.princess;
...
player.boy = 'images/char-boy.png';
